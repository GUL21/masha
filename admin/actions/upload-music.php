...
if($_FILES['form_music']['type'] == 'audio/mp3' || $_FILES['form_music']['type'] == 'audio/wav')
{ 
$imgext = strtolower(preg_replace("#.+\.([a-z]+)$#i", "$1", $_FILES['form_music']['name']));
    //папка для загрузки
$uploaddir = '../sound/';
...
if (move_uploaded_file($_FILES['form_music']['tmp_name'], $uploadfile))
{
  $update = mysql_query("UPDATE celebrations SET music='$newfilename' WHERE cel_id = '$id'",$link);   
}
else
{
...
{
 $error_img[] =  'Допустимые расширения: mp3, wav';
}
