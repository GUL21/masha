...
if($_FILES['form_img']['type'] == 'image/jpeg' || $_FILES['form_img']['type'] == 'image/jpg' || $_FILES['form_img']['type'] == 'image/png' || $_FILES['form_img']['type'] == 'image/gif')
{ 
$imgext = strtolower(preg_replace("#.+\.([a-z]+)$#i", "$1", $_FILES['form_img']['name']));
    //папка для загрузки
$uploaddir = '../logos/';
...
if (move_uploaded_file($_FILES['form_img']['tmp_name'], $uploadfile))
{
  $update = mysql_query("UPDATE celebrations SET img='$newfilename' WHERE cel_id = '$id'",$link);   
}
else
{
...
{
 $error_img[] =  'Допустимые расширения: jpeg, jpg, png, gif';
}
